...
export default function AppSidebar({ active }: { active: string }) {
  ...
  return <aside className={`sidebar slick-sidebar ${collapsed ? "collapsed" : ""}`}>
    <div className="side-top"><a className="brand" href="/"><span className="brand-mark">d</span><span className="side-copy">distroa</span></a><button className="sidebar-toggle" onClick={() => setCollapsed(!collapsed)} aria-label={collapsed ? "Open navigation" : "Collapse navigation"}><span className="panel-icon"/></button></div>
